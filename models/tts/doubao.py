"""Doubao / Volcano Engine TTS playback with pre-fetching for gapless sentence transitions.

API docs: https://www.volcengine.com/docs/6561/80818
...
class TTSPlayer:
    ...
    def _fetch_wav(self, text: str) -> bytes | None:
        """Fetch WAV audio from Volcano Engine TTS V1 HTTP API."""
        url = "https://openspeech.bytedance.com/api/v1/tts"
        headers = {
            "Authorization": f"Bearer;{config.DOUBAO_ACCESS_TOKEN}",
            "Content-Type": "application/json",
        }
        payload = {
            "app": {
                "appid": config.DOUBAO_APPID,
                "token": "access_token",
                "cluster": "volcano_tts",
            },
            "user": {"uid": "openclaw"},
            "audio": {
                "voice_type": config.DOUBAO_VOICE_TYPE,
                "encoding": "wav",
                "speed_ratio": 1.0,
            },
            "request": {
        ...
        try:
            ...
            if not audio_b64:
                print(f"[tts] empty audio data in response: {resp.text[:200]}")
                return None
            wav_data = base64.b64decode(audio_b64)
        except (ValueError, KeyError) as e:
            print(f"[tts] response parsing failed: {e}")
            return None
